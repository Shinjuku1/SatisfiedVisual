...
export function initializeEventListeners() {
    ...
    dom.collapseAllBtn.addEventListener('click', () => {
        dom.library.querySelectorAll('.collapsible-header.active').forEach(header => {
...
// --- Connection Helpers ---
export function completeConnection(startNode, endNode) {
    if (startNode && endNode && startNode !== endNode) {
        ...
        if (startCardId !== endCardId && startNode.dataset.type === 'output' && endNode.dataset.type === 'input' && startItem === endItem) {
            ...
            state.connections.set(connId, {
                from: { cardId: startCardId, itemName: startItem },
                to: { cardId: endCardId, itemName: endItem }
            });
...
export function cancelConnectionDrawing() {
...
function handleCanvasMouseDown(e) {
    ...
    // If the click is on any part of a card, let the card's own handler deal with it.
    if (e.target.closest('.placed-card')) return;

    // Pan with middle mouse, right mouse, or spacebar + left mouse
    if (e.button === 1 || e.button === 2 || (e.button === 0 && state.panning)) {
        e.preventDefault();
        state.panning = true;
        ...
        // This block now only handles left-clicks for selection
        // It won't run if a card was clicked due to the check at the top.
        if (!e.shiftKey && !e.ctrlKey) {
            state.selectedCardIds.clear();
            state.selectedConnectionIds.clear();
            renderCardSelections();
